...
template<typename T>
inline detail::tquat<T> make_quat(const T angle, const detail::tvec3<T> &v)
{
    typename detail::tquat<T>::value_type hsin = sin(angle * T(0.5));
	return detail::tquat<T>(cos(angle * T(0.5)), v.x * hsin, v.y * hsin, v.z * hsin);
}
...
{
    return reinterpret_cast<const quat&>(_mm_cross4_ps(
    ...
    detail::tvec3<T>& bitangent)
{
    ...
    detail::tvec3<T> vN = cross(vA, vB);
    vN = normalize(cross(vA, vB));
    
    detail::tvec3<T> vU, vV;
    ...
    const bool camera = false)
{
    ...
    if(!camera)
        qr = detail::tquat<T>(M_SQRT1_2, M_SQRT1_2, 0, 0) * qr;
    ...
    const bool camera = false)
{
    ...
    const detail::tquat<T>& qs = camera
        ? q
        : (qm = detail::tquat<T>(M_SQRT1_2, -M_SQRT1_2, 0, 0)) * q;
    ...
    const bool camera = false)
{
    ...
    if(!camera)
        qr = qr * glm::detail::tquat<T>(M_SQRT1_2, -M_SQRT1_2, 0, 0);
